...
middleware.linientIsJwtValid = (req, res, next) => {
    let token;
    ...
    if(token){
        ...
        tokenObj = utilities.jwt('v', token);
        if (!tokenObj.isVerified) {
            utilities.setResponseData(res, 401, { 'content-type': 'application/json' }, { msg: 'Unauthorized' },true);
            return;
        }
    ...
    req.decodedToken = tokenObj?.decodedToken || null; // If tokenObj is undefined, set decodedToken to null
    next();
};
...
middleware.isUserDeleted = async (req, res, next)=>{
