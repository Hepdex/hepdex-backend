...
savedJobController.getSavedJob = ("/get-saved-job", async (req, res)=>{
    try{
        const userID = ObjectId.createFromHexString(req.decodedToken.userID)
        const savedJobID = ObjectId.createFromHexString(req.query.savedJobID)
       
        //get all saved jobs for the user
        let savedJob = await database.db.collection(database.collections.savedJobs).aggregate([
            {
                $match: {
                    _id: savedJobID,
                    userID: userID,
        ...
        //send response
        utilities.setResponseData(res, 200, {'content-type': 'application/json'}, {savedJob}, true)
        return
